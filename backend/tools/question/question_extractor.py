...
from backend.services.llm.capabilities import supports_response_format
...
def load_parsed_paper(paper_dir: Path) -> tuple[str | None, list[dict] | None, Path]:
    ...
    if json_files:
        ...
        print("Warning: content_list.json file not found, will use markdown content only")

    images_dir = auto_dir / "images"
    if images_dir.exists():
        image_count = len(list(images_dir.glob("*")))
        print(f"Found image directory: {image_count} images")
    else:
