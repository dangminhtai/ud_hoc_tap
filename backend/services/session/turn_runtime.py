...
from backend.services.session.sqlite_store import SQLiteSessionStore, get_sqlite_session_store

logger = logging.getLogger(__name__)


def _should_capture_assistant_content(event: StreamEvent) -> bool:
    """Should capture assistant content."""
...
def _extract_followup_question_context(
    ...
    """Extract followup question context."""
...
def _format_followup_question_context(context: dict[str, Any], language: str = "en") -> str:
    """Format followup question context."""
...
class _LiveSubscriber:
    """Live subscriber class."""
    queue: asyncio.Queue[dict[str, Any]]


@dataclass
class _TurnExecution:
    """Turn execution class."""
    turn_id: str
    session_id: str
    capability: str
...
class TurnRuntimeManager:
    ...
    async def start_turn(self, payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        try:
            from backend.capabilities.request_contracts import validate_capability_config
    ...
    async def _run_turn(self, execution: _TurnExecution) -> None:
        ...
        try:
            from backend.core.context import Attachment, UnifiedContext
            from backend.runtime.orchestrator import ChatOrchestrator
            from backend.agents.notebook import NotebookAnalysisAgent
            from backend.services.memory import get_memory_service
            from backend.services.notebook import notebook_manager
            from backend.services.llm.config import get_llm_config
            from backend.services.session.context_builder import ContextBuilder
...
def get_turn_runtime_manager() -> TurnRuntimeManager:
    """Return the turn runtime manager."""
