...
from backend.services.config import get_config_test_runner, get_model_catalog_service
from backend.services.embedding.client import reset_embedding_client
from backend.services.llm.client import reset_llm_client
from backend.services.llm.config import clear_llm_config_cache
...
class SidebarDescriptionUpdate(BaseModel):
    """Sidebar description update class."""
    description: str


class SidebarNavOrderUpdate(BaseModel):
    """Sidebar nav order update class."""
    nav_order: SidebarNavOrder


class CatalogPayload(BaseModel):
    """Catalog payload class."""
...
async def stream_service_test_events(service: str, run_id: str, request: Request):
    """Stream service test events asynchronously."""
...
async def complete_tour(payload: TourCompletePayload | None = None):
    ...
    return {
        "status": "completed",
        "message": "Configuration saved. backend will restart shortly.",
        "launch_at": launch_at,
