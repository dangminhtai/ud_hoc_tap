...
from backend.services.config import resolve_search_runtime_config
...
from backend.services.llm import get_llm_config, get_token_limit_kwargs
...
async def test_search_connection():
    ...
    try:
        ...
        result = web_search("backend health check", provider=search_config.provider)
        response_time = (time.time() - start_time) * 1000
