...
from backend.core.trace import build_trace_metadata, merge_trace_metadata, new_call_id


class MathAnimatorCapability(BaseCapability):
    """Math animator capability class."""
    ...
    async def run(self, context: UnifiedContext, stream: StreamBus) -> None:
        ...
        if importlib.util.find_spec("manim") is None:
            raise RuntimeError(
                ...
                "Install with `pip install 'backend[math-animator]'` "
                "or `pip install -r requirements/math-animator.txt`."
            )
        from backend.agents.math_animator.pipeline import MathAnimatorPipeline
